...
export default function DeleteTaskModal({ taskNumber = 0, onSubmit, modal, onClose }) {
	return (
		<React.Fragment>
			<Dialog
				open={modal}
				...
				<DialogContent className="w-80  flex flex-col justify-center items-center gap-3 ">
					...
					<Box sx={customStyle.fontBlack} className="flex gap-1.5">
						{taskNumber > 0 && (
							<Typography color="primary" sx={customStyle.mainBtn}>
								{taskNumber}
							</Typography>
						)}
						{taskNumber > 1 ? 'Tasks' : 'Task'} will be deleted.
					</Box>
					{/* </DialogContentText> */}
				</DialogContent>
